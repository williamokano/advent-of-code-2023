...
// I hate oneight and twone
var numericStrings = []string{"zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"}
...
func parseDigit(in string) string {
	if value, ok := mapping[in]; ok {
		return value
	}
	return in
}
...
func parseLine(line string) int {
	...
	// Just a hack, I'm sorry gods of programming
outerLoop:
	for idx, char := range line {
		...
		for _, numericString := range numericStrings {
			if strings.HasPrefix(line[idx:], numericString) {
				matches = append(matches, numericString)
				continue outerLoop
			}
		}
	}

	firstDigit := parseDigit(matches[0])
	lastDigit := parseDigit(matches[len(matches)-1])
