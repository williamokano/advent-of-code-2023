...
const (
	RedCube   Cube = "red"
	BlueCube  Cube = "blue"
	GreenCube Cube = "green"
)

var coloredCubes = []Cube{RedCube, BlueCube, GreenCube}

type Cubes map[Cube]int

func parseCubes(input string) ([]Cubes, error) {
	picks := strings.Split(input, ";")
	var response = make([]Cubes, len(picks))

	for idx, pick := range picks {
		cubes := strings.Split(strings.Trim(pick, " "), ",")
		parsedCube := Cubes{
			RedCube:   0,
			BlueCube:  0,
			GreenCube: 0,
		...
		for _, cube := range cubes {
			...
			quantity, err := strconv.Atoi(cubeParts[0])
			if err != nil {
				return response, err
			}

			parsedCube[Cube(cubeParts[1])] = quantity
		}

		response[idx] = parsedCube
	}

	return response, nil
}

func parseLine(line string) (int, []Cubes, error) {
	var gameID int
	var cubes = make([]Cubes, 0)

	parts := strings.Split(line, ": ")
	if len(parts) != 2 {
		return gameID, cubes, errors.New("too many parts on line")
	}

	gameID, err := strconv.Atoi(parts[0][5:])
	if err != nil {
		return gameID, cubes, fmt.Errorf("failed to get game id: %v", err)
	}

	cubes, err = parseCubes(parts[1])
	if err != nil {
		return gameID, cubes, fmt.Errorf("failed to parse cubes pickup: %v", err)
	}

	return gameID, cubes, nil
}

func totalCubes(cubes Cubes) int {
	var total = 0
	for _, color := range coloredCubes {
		total += cubes[color]
	}
...
func isPossibleGame(sackConfig Cubes, picks []Cubes) bool {
	...
	for _, pick := range picks {
		...
		for _, color := range coloredCubes {
			if pick[color] > sackConfig[color] {
				return false // Not possible as the pick is bigger than the existent in the sack
			}
		}

		// Check for total pick
		if totalCubes(pick) > maxPossibleCubes {
			return false // Not possible as it was shown more cubes than there's is in the bag
		}
	}
