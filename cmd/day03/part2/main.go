...
var gearMarker = regexp.MustCompile(`\*`)
var endingWithNumber = regexp.MustCompile(`\d+$`)
var startsWithNumber = regexp.MustCompile(`^\d+`)
var getNumbers = regexp.MustCompile(`\d+`)

func intersect(range1 []int, range2 []int) bool {
	if len(range1) != 2 || len(range2) != 2 {
		panic(errors.New("range can only have 2 items"))
	}
	start1, end1 := range1[0], range1[1]
	start2, end2 := range2[0], range2[1]
	// Using < instead of <= for ending as in golang the slice upper bound is exclusive [0:4] means 0, 1, 2, 3
	return (start1 <= start2 && start2 < end1) || (start2 <= start1 && start1 < end2)
}

func getNumbersAround(index int, lineNr int, lines []string) []int64 {
	stringBefore := lines[lineNr][:index]
	stringAfter := lines[lineNr][index+1:]
	verticalBoundaries := []int{index - 1, index + 2}
	lineAbove := ""
	if lineNr > 0 {
		lineAbove = lines[lineNr-1]
	}
	lineBelow := ""
	if lineNr+1 < len(lines) {
		lineBelow = lines[lineNr+1]
	}

	fmt.Println(lineAbove)
	fmt.Println(fmt.Sprintf("%s%s%s", stringBefore, "*", stringAfter))
	fmt.Println(lineBelow)
	var numbersAround []int64

	// Check left
	if strFound := endingWithNumber.FindString(stringBefore); strFound != "" {
		conv, err := strconv.ParseInt(strFound, 10, 64)
		if err != nil {
			panic(err)
		}
		numbersAround = append(numbersAround, conv)
	}

	// Check right
	if strFound := startsWithNumber.FindString(stringAfter); strFound != "" {
		conv, err := strconv.ParseInt(strFound, 10, 64)
		if err != nil {
			panic(err)
		}
	...
	// Check above
	if numbersAbove := getNumbers.FindAllStringSubmatchIndex(lineAbove, -1); len(numbersAbove) > 0 {
		for _, numberAbove := range numbersAbove {
			if intersect(numberAbove, verticalBoundaries) {
				strFound := lineAbove[numberAbove[0]:numberAbove[1]]
				conv, err := strconv.ParseInt(strFound, 10, 64)
				if err != nil {
					panic(err)
				}
	...
	// Check below
	if numbersBelow := getNumbers.FindAllStringSubmatchIndex(lineBelow, -1); len(numbersBelow) > 0 {
		for _, numberBelow := range numbersBelow {
			if intersect(numberBelow, verticalBoundaries) {
				strFound := lineBelow[numberBelow[0]:numberBelow[1]]
				conv, err := strconv.ParseInt(strFound, 10, 64)
				if err != nil {
					panic(err)
				}
...
func product(input []int64) int64 {
	var total int64 = 1
...
func ans(input string) {
	...
	for lineNr, line := range lines {
		gearMarkers := gearMarker.FindAllStringSubmatchIndex(line, -1)
		for _, gearMarkerPosition := range gearMarkers {
			numbersAround := getNumbersAround(gearMarkerPosition[0], lineNr, lines)
			fmt.Println(numbersAround)
			if len(numbersAround) > 1 {
				total = total + product(numbersAround)
			}
		}
	}
...
func main() {
	//fmt.Println(getNumbersAround(5, 8, strings.Split(testInput, "\n")))
	ans(testInput)
	ans(input)
}
