...
var numberRegex = regexp.MustCompile(`\d+`)     // Get all numbers
var regexSymbol = regexp.MustCompile(`[^\d|.]`) // Get everything that is not a number or a dot
...
func ans(input string) {
	...
	for lineNr, line := range lines {
		indices := numberRegex.FindAllStringSubmatchIndex(line, -1)
		for _, index := range indices {
			...
			// Check left
			if start > 0 && regexSymbol.MatchString(line[start-1:start]) {
				total += value
				continue
			}

			// check right
			if end+1 <= len(line) && regexSymbol.MatchString(line[end:end+1]) {
				total += value
				continue
			}
			// check top with corners
			boundaryStart := MaxInt(0, start-1)
			boundaryEnd := MinInt(len(line), end+1)

			if lineNr > 0 && regexSymbol.MatchString(lines[lineNr-1][boundaryStart:boundaryEnd]) {
				total += value
				continue
			}

			// check bottom with corners
			if lineNr+1 < len(lines) && regexSymbol.MatchString(lines[lineNr+1][boundaryStart:boundaryEnd]) {
				total += value
				continue
			}
		}
